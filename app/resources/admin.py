...
class Admin_Crud(Resource):
	...
	def get(self):
		...
		if not pagination.items:
			...
			admin_logger.info(f'All admin data shown to Admin[id : {get_jwt_identity()}]')
			return {
			'admins': admins,
		...
		if errors:
			...
			if check_user:
				...
				try:
					...
					db.session.commit()
					admin_logger.info(
						f'{username_new_admin} has been added as new admin by Admin[id : {get_jwt_identity()}]')
	...
	def put(self, id):
		...
		if not check_user:
			...
			try:
				...
				db.session.commit()
				admin_logger.info(
					f'{check_user.username} : promoted to user -> Admin by Admin[id : {get_jwt_identity()}]')
	...
	def delete(self, id):
		...
		if not check_user:
			...
			try:
				...
				db.session.commit()
				admin_logger.info(f'{check_user.username} has been removed from admin by Admin[id : {get_jwt_identity()}]')
				return {
...
class Admin_Book_Manage(Resource):
	...
	def get(self):
		...
		if not pagination.items:
			...
			admin_logger.info(f'All book data sent to Admin[id : {get_jwt_identity()}]')
			return {
			'user_id' : user_id,
			'books': books,
			'page': pagination.page,
...
class User_Show(Resource):
	...
	def get(self):
		...
		if not pagination.items:
			...
			admin_logger.info(f'All user data sent to Admin[id : {get_jwt_identity()}]')
			return {
			'users': users,
			'page': pagination.page,
			'per_page': pagination.per_page,
...
class User_Control(Resource):
	...
	def delete(self, id):
		...
		try:
			...
			db.session.commit()
			admin_logger.info(f'User [{check_user.username}] has been banned by Admin[id : {get_jwt_identity()}]')
	...
	def put(self, id):
		...
		try:
			...
			db.session.commit()
			admin_logger.info(f'User [{check_user.username}] has been unbanned by Admin[id : {get_jwt_identity()}]')
...
class UserCredChange(Resource):
	...
	def post(self, data):
		...
		if not username_of_user or not email or not new_pass:
			...
			try:
				db.session.commit()
				admin_logger.info(f'User [{username_of_user}] password has been changed by Admin[id : {get_jwt_identity()}]')
...
class Jwt_Manage(Resource):
	...
	def delete(self):
		...
		try:
			db.session.commit()
			admin_logger.info(f"JWT token blacklist database has been cleared by Admin[id : {get_jwt_identity()}]")
		except SQLAlchemyError as e:
			db.session.rollback()
			raise e
						 
		if is_deleted:
			return {"message" : "All expired JWT tokens have been deleted."}, 200
		else:
			return {"message": "No expired JWT tokens found for deletion."}, 403


class BookManage(Resource):
	# Deleting soft deleted books.
	@jwt_required()
	@admin_required
	def delete(self):
		from app.models import book_manager
		
		deleteable_books = book_manager.query.filter(
			book_manager.is_deleted == True).all()
		now = datetime.utcnow()
		deleted_any = False

		for book in deleteable_books:
			if now >= (book.updated_at + timedelta(days=30)):
				db.session.delete(book)
				deleted_any = True
		try:
			db.session.commit()
			admin_logger.info(f"Soft deleted books have been deleted by Admin[id : {get_jwt_identity()}].")
		except SQLAlchemyError as e:
			db.session.rollback()
			raise e
...
class UserAccDelete(Resource):
	...
	def delete(self):
		...
		try:
			db.session.commit()
			admin_logger.info(f"Following users have been removed from the db permanently: {', '.join(user_info)} by Admin[id : {get_jwt_identity()}].")
		except SQLAlchemyError as e:
			db.session.rollback()
			raise e
...
class UserStat(Resource):
	...
	def get(self):
		...
		admin_logger.info(f"User's Stat has been shown to Admin[id : {get_jwt_identity()}].")
		return {
