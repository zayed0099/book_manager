...
class PublisherUD(Resource):
	...
	def patch(self, id):
		...
		if publisher is None and not isinstance(publisher, str):
			return {"message" : "Publisher name is required in Str format."}, 400
	...
	def put(self, id):
		...
		try:
			...
			db.session.commit()
			return {'message' : 'Publisher status successfully updated.'}, 200
		except SQLAlchemyError as e:
			db.session.rollback()
			return {'message' : 'An error occured'}, 500